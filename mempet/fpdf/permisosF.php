...
$act = pg_query("SELECT  id_permiso, cedula, tipo, motivo, fecha_ini, fecha_cul from tbl_permisos WHERE cedula=20344656  ORDER BY cedula");
...
function tabla1($header,$data)
    ...
    $this->SetLineWidth(.2);    // ancho de linea
    $this->SetFont('','B');     // negrita
    $w1=array(10,20,35,40,35,45);   // en este arreglo definiremos el ancho de cada columna
...
$pdf->AliasNbPages(); //funcion que calcula el numero de paginas
$head1 = array("Id", "Cedula","Tipo de Permiso","Motivo del Permiso","Fecha de Inicio","Fecha de Culminacion"); // cabecera
$i=0;
while ($fila=pg_fetch_array($act)){  //llenar variable dat con los campos de una fila unidos por ;
    $dat1[$i]=$fila[0].";".($fila[1]).";".$fila[2].";".$fila[3].";".$fila[4].";".$fila[5]; //concatenar para luego ser separado por explode()
    $i++;
