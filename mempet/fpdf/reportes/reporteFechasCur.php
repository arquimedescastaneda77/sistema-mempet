...
$SQL="SELECT tbl_persona.cedula,tbl_persona.nombrec,titulo, lugar, duracion,curso_ini,curso_fin from tbl_capacitacion INNER JOIN tbl_persona ON tbl_persona.cedula=tbl_capacitacion.cedula";
if(isset($_REQUEST["curso_ini"])){
        $date1=pg_escape_string($_REQUEST["curso_ini"]);
        $SQL.=" WHERE (curso_ini>=DATE('{$date1}')";
        if(isset($_REQUEST["curso_fin"])){
            if(!empty(trim($_REQUEST["curso_fin"]))){
                $date2=pg_escape_string($_REQUEST["curso_fin"]);
                $SQL.=" AND curso_fin<=DATE('{$date2}')";               
            }
        }
        $SQL.=")";
}
...
class PDF extends FPDF {
    ...
    $w=array(25,40,50,45,30,30,30);   // en este arreglo definiremos el ancho de cada columna
    ...
    $fill=false;  // variable para alternar relleno
        ...
        foreach($data as $row)
        {
            $columna = explode(";",$row); //separar los datos en posiciones de arreglo
                ...
                $this->Cell($w[6],8,$columna[5],'LR',0,'C',$fill);
            $this->Ln();
...
$head = array(utf8_decode("Cédula"),"Trabajador",utf8_decode("Título"),"Lugar",utf8_decode("Duración"),"F.Inicio",utf8_decode("F.culminación")); // cabecera
$dat=[];

while ($fila=pg_fetch_array($act)){  //llenar variable dat con los campos de una fila unidos por ;
 $dat[]=$fila[0].";".($fila[1]).";".($fila[2].";".$fila[3].";".$fila[4].";".$fila[5].";".$fila[6]); //concatenar para luego ser separado por explode()
}
...
$pdf->Cell(180,10,'REPORTES DE CURSOS REALIZADO POR EL PERSONAL',0,1,'L');//0 para misma linea, 1 para salto de linea
$pdf->Ln(10);
...
if(count($dat)>0){
        ...
        $pdf->Cell(200,8,'NO HAN SOLICITADO CURSOS EN ESE LAPSO DE TIEMPO',0,1,'C');
    }
